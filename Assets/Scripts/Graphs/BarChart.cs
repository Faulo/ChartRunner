...
public class BarChart : MonoBehaviour
{
    ...
    private void Start() {
        foreach (var bar in this.gameObject.GetComponentsInChildren<Transform>()) {
    ...
    public void AddBarPart() {
        var newBarPart = Instantiate(barPartPrefab, this.transform.position, Quaternion.identity);
        newBarPart.transform.SetParent(this.transform);
    ...
    public void CalculateGraphParts() {
        ...
        foreach (var barPart in barParts) {
            ...
            if (counter > 0) {
                barPart.transform.position = new Vector3(this.transform.position.x, barParts[counter-1].GetComponent<BarPartVisualization>().topPivot.position.y, 0);
            }
